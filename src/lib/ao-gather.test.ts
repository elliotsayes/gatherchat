...
describe("AoGatherProvider", () => {
  ...
  it("should be able to get all rooms", () => {
    const aoGatherProvider = getTestInstance()
    expect(aoGatherProvider.getRoom()).resolves.toMatchSnapshot()
  })

  it("should be able to get a specific room", () => {
    const aoGatherProvider = getTestInstance()
    expect(aoGatherProvider.getRoom({ roomId: "WelcomeLobby" })).resolves.toMatchSnapshot()
  })
  ...
  it("should be able to get posts from a specific room", () => {
    const aoGatherProvider = getTestInstance()
    expect(aoGatherProvider.getPosts({ roomId: "WelcomeLobby" })).resolves.toMatchSnapshot()
  })
})