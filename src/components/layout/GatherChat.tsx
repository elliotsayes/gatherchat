    ...
    const renderEngineState: RenderEngineState = useMemo(() => {
        ...
        return {
            ...
            player: {
                ...
                profile: player,
                savedPosition:
                    contractState.room.playerPositions[playerAddress],
            },
            otherPlayers: Object.entries(contractState.users)
                ...
                .map(([address, otherPlayer]) => {
                    return {
                        ...
                        profile: otherPlayer,
                        savedPosition:
                            contractState.room.playerPositions[address],

                        // Derived
                        isFollowingUser: Object.keys(
                            otherPlayer.following,
                        ).includes(address),
                        isFollowedByUser: Object.keys(
                            player.following,
                    ...
                    activityFeed={
                        <div className="min-h-min h-auto flex flex-col gap-4 py-4">
                            <ul className="w-[100%] min-h-0 max-h-full h-[calc(100vh-140px)] overflow-y-auto px-2">
                                {Object.keys(contractState.posts).map(
                                    (postId) => {
                                        const post =
                                            contractState.posts[postId];
                                        const selectedPlayer = [
                                            ...
                                            | undefined;
                                        const isUser =
                                            renderEngineState.player.id ===
                                            post.author;
                                        const isLink =
                                            !isUser && selectedPlayer;
                                        return (
                                            <li
                                                key={postId}
                                                className={`${
                                            ...
                                            >
                                                <button
                                                    className={
                                                        "text-muted-foreground text-underline px-1"
                                                    }
                                                    type="button"
                                                    onClick={
                                                        isLink
                                                            ? () => {
                                                                  selectAndAnimatePlayer(
                                                                      selectedPlayer,
                                                                  );
                                                                  setSidePanelState(
                                                                      "profile",
                    ...
                    profile={
                        selectedPlayer ? (
                            ...
                            <SetupForm
                                ...
                                initialUsername={
                                    renderEngineState.player.profile.name
                                }
                                initialSeed={
                                    renderEngineState.player.profile.avatar
                                }
                            />
                        )
                    }
